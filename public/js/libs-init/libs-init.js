...
CRUMINA.Bootstrap = function() {
    ...
    var e = $('input[name="datetimepicker"]');
    if (e.length) {
    ...
    $(".sorting-container").each(function() {
        var e = $(this),
            i = e.data("layout").length ? e.data("layout") : "masonry";
        e.isotope({
    ...
    init: function() {
        for (var e in this.maps) {
            ...
            if (i.config && i.markers && document.getElementById(i.config.id)) {
                var a = new L.map(i.config.id, i.config.map),
                    t = L.markerClusterGroup({
                        iconCreateFunction: function(a) {
                            var t = a.getChildCount(),
                                n = i.config.cluster;
